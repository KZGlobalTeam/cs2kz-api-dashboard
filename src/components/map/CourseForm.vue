<template>
  <div>
    ...
    >
      ...
      <n-select
        multiple
        v-model:value="mapperSteamIds"
        :options="mapperOptions"
      />

      <!-- Filters [New Filter] -->
      <div class="flex items-center justify-between border-b border-slate-600 pb-2 mt-4 mb-2">
        <p>Filters</p>
      ...
      <n-table :bordered="false" :single-line="false">
        ...
        <tbody>
          <tr v-for="(filter, filterIndex) in course.filters">
            <td>
...
const mapperSteamIds = ref([])

const mapperOptions = courses.value
  .flatMap((course) => course.mappers)
  .map((mapper) => {
    return {
      label: mapper.name,
