<template>
  <div>
    <div class="p-4 bg-gray-800 mb-4 rounded-md">
      ...
      <div>
        <n-button @click.prevent="updateServer" :disabled="loading" :loading="loading" class="saveButton"
    ...
    <div class="p-4 bg-gray-800 rounded-md">
      ...
      <div class="flex gap-4">
        <n-button @click="newKey" text-color="#3cc962" style="font-size: 16px" strong>
          New Key
        </n-button>
        <n-button @click="revokeKey" type="error" textColor="#e2e8f0" style="font-size: 16px" strong>
          Revoke Key
        </n-button>
      </div>
    </div>
...
onBeforeMount(async () => {
  if (route.params.id) {
    try {
      ...
      notification.error({ title: 'Failed to fetch the server', content: toErrorMsg(error) })
    }
  }
})
...
async function updateServer() {
  if (isEqual(oldServer, toRaw(server))) {
    ...
    try {
      await axiosClient.patch(`/servers/${route.params.id}`, update, { withCredentials: true })
      notification.success({ title: 'Server updated', duration: 3000 })
      router.push("/home/servers")
    } catch (error) {
      loading.value = false
      notification.error({ title: 'Failed to update server', content: toErrorMsg(error) })
    }
  }
}
